...
const malHandler = {
  ...
  formatMetadata(anime, episodes, seasonNumber) {
...
function getMetadataPath() {
  ...
  return join$1(userDataPath, "metadata.json");
}
...
function getConfigPath() {
  const userDataPath = app.getPath("userData");
  return join$1(userDataPath, "config.json");
}
...
const configHandler = {
  ...
  async removeFolderSource(folderPath) {
    try {
      const config = await this.loadConfig();
...
function createWindow() {
  ...
  mainWindow = new BrowserWindow({
    ...
    minHeight: 600,
    webPreferences: {
      preload: join$1(__dirname$1, "preload.js"),
...
app.whenReady().then(() => {
  protocol.handle("media", async (request2) => {
    ...
    try {
      ...
      if (mimeTypes2[ext]) {
        newHeaders.set("Content-Type", mimeTypes2[ext]);
      }
...
ipcMain.handle("get-series-episodes", async (_event, seriesId) => {
  try {
